...
class TestAppService(BaseTestCase):
    ...
    @db_session
    def test_all_matches_by_date(self):
        """Ensure get matches by date behaves correctly."""
        match = select(m for m in Match if m.start_date_time.day == 11)[:]

        with self.client:
            response = self.client.get(f'/matches/07-11-2020')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertEqual(len(data['data']), len(match))
